...
def _repack_tasks_into_waves(tasks: List[PredictionTask],
    ...
    Used by ``run_temporal_wave_batch`` when the anchor has finished
...
def run_temporal_wave_batch(
    ...
    **Phase 1 — in-shadow execution.** Launch the anchor task(s), then
    ...
    ``batch.wave_memory_budget`` = effective VRAM - anchor footprint,
    so it co-exists safely with the anchor. Waves are dispatched
    ...
    *repacked* against the **full** effective VRAM
    ...
    preserved across the repack. This fixes the silent task-loss bug
    where un-dispatched wave tasks vanished from the result list and
    the per-task retry pass never saw them.

    A defensive backstop at the end of the function marks any wave task
    still missing from the result list as failed so the per-task retry
    pass in ``run_gpu_worker`` can recover it. This guarantees no
    task in a TemporalWaveBatch can ever silently disappear, even under
    future scheduling regressions.
    ...
    # Defensive guard: a TemporalWaveBatch with no anchor is malformed
    # (the planner never emits one). We refuse to dispatch rather than
    # raising deep inside ThreadPoolExecutor(max_workers=0). Returning
    # an empty result list lets the run_gpu_worker accounting stay
    # consistent — every wave task in this batch is marked failed via
    # the backstop below and routed through the per-task retry pass.
    if not anchor_tasks:
        warning(f"[GPU{gpu_id}] {batch.batch_id}: TemporalWaveBatch has no "
                f"anchor tasks; marking all {total_wt} wave task(s) failed "
                f"for the retry pass.")
        results: List[Tuple[PredictionTask, bool, float, int]] = []
        for w in waves:
            for t in w.tasks:
                results.append((t, False, 0.0, 0))
                if on_task_complete:
                    on_task_complete(t, False, 0.0, 0, gpu_id, batch, w.wave_id)
        return results
    ...
    def _dispatch_wave(executor, wave: TaskWave, wave_id_for_log: str) -> None:
    ...
    remaining_waves: List[TaskWave] = []     # waves not dispatched in phase 1
    ...
    # ── Backstop: any wave task still without a result is marked failed
    # so run_gpu_worker's per-task retry pass can recover it. With the
    # phase-2 repack this should be a no-op in practice, but it guards
    # against future exception paths or scheduling regressions.
    seen_ids = {t.task_id for t, *_ in all_results}
    missing = [(tid, pair) for tid, pair in expected_wave_tasks.items()
...
def run_gpu_worker(worker: GPUWorker, singularity_image: str,
    ...
    def on_task_complete(task, ok, runtime, peak_mem, gid, batch, wave_id=''):
        ...
        if isinstance(batch, TemporalWaveBatch):
            if wave_id == 'anchor':
                btype = 'temporal_anchor'
            elif '_post' in wave_id:
                # Post-anchor repacked wave (phase 2 of run_temporal_wave_batch).
                # Tagged distinctly so downstream analyses can compute the
                # fraction of tasks that ran inside the anchor's VRAM
                # shadow vs on the freed GPU after the anchor finished.
                btype = 'temporal_wave_post'
            else:
                btype = 'temporal_wave'
        else:
            btype = 'normal'
